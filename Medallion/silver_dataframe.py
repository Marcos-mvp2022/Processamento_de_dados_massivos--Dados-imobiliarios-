...
def build_silver_tables(bronze_paths: List[str], outdir: str):
    ...
    if "pricingInfos_arr" in dfb.columns:
        # Explode para criar uma linha por item de pricingInfos_arr
        ...
        if not dfp_temp.empty and any(isinstance(x, dict) for x in dfp_temp["pricingInfos_arr"].dropna()):
            # A coluna explodida é uma série de dicionários. Normaliza (desaninha)
            ...
            # Concatena o 'id' (agora 'listing_id') com as colunas desaninhadas
